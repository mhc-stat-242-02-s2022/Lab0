...
In this lab, we will be reviewing some basic concepts of R, as well as extending the work we were doing by hand on the Creativity study in the previous class exercise. Using R will allow us to carry out larger simulation studies than we can achieve by hand, so the results will be more reliable.
...
library(dplyr)
library(ggplot2)
library(Sleuth3) ## this is the R package with datasets from our textbook
```
...
```{r}
creativity <- case0101 # case0101 is loaded with Sleuth3 package
head(creativity)
```
...
You may recall that you can look up what functions in R do by typing ? followed by the name of the function. The code above used a function called shuffle, which you have likely not encountered before. If you have extra time, run the following code chunk and read the help documentation for shuffle. What does this function do?
